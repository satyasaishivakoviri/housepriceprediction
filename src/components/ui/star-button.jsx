...
export function StarButton({
    ...
    duration = 3,
    lightColor = "#FAFAFA",
    backgroundColor = "transparent", // Keep transparent for StarBackground, but button should be black
    borderWidth = 1,
    className,
    ...props
}) {
    ...
    return (
        <button
            ...
            ref={pathRef}
            className={cn(
                "relative z-[3] overflow-hidden h-10 px-4 py-2 inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-3xl text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50 group/star-button bg-black/50",
                className
            )}
            {...props}
        >
            ...
            />
            <div
                className="absolute inset-0 border-white/20 z-[1] overflow-hidden rounded-[inherit] text-white"
                style={{ borderWidth: "var(--border-width)" }}
                aria-hidden="true"
            >
                <StarBackground color={backgroundColor} />
            </div>
            <span className="z-10 relative inline-block font-semibold text-white">
                {children}
            </span>
        </button>
    );
